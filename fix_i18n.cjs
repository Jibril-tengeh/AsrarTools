...
// EXACT CATEGORIES NO ROMAN NUMERALS
const catFR = {
...
Object.assign(enTranslations, catFR); // Using same key names for EN but keeping French fallback, let's just make it EN
for (let key in catFR) {
  enTranslations[key] = catFR[key]; // Copying for now to avoid crashes
}
